...
<html>
<head>
<title>${initParam['AppName']} - PasoGuiaTramites</title>
...
<style>#mostradorPasoGuiaTramite {
  width:0px;
  height:0px;
  display:none;
}
...
<form name="formBuscar" method="get" action="search" class="buscador">
  ...
  <br/><br/>
  <a class="searchbutton" onclick="displayMostrador('#mostradorPasoGuiaTramite','#btnMostrarPasoGuiaTramite','60px');" id="btnMostrarPasoGuiaTramite">Datos PasoGuiaTramite</a>
  <a class="searchbutton" onClick="document.forms[0].vista.value='';document.forms[0].submit();"> Mostrar </a>
  </p>

    <div id="mostradorPasoGuiaTramite">
    <br/>
      <p><b>
       <a onclick="hideMostrador('#mostradorPasoGuiaTramite','#btnMostrarPasoGuiaTramite');" id="btnHideMostrar">
            <img alt="Ocultar Datos" width="12px" height="12px" src="<c:url value='/images/icons/cross.png'/>"/>
          </a>
        Datos Paso Guia Tramite:</b>
...
<div id="scrollable">
  <table id="listado" id="myTable" class="tablesorter" border="0" cellpadding="0" cellspacing="1">
    ...
    <tbody>
      <c:if test='${empty pasoGuiaTramites}'>
        <tr>
          <td colspan="5"><br>
          <br>
          <h3>No se encontraron pasoGuiaTramites</h3>
          </td>
        </tr>
      </c:if>
      <c:forEach items="${pasoGuiaTramites}" varStatus="status" var="bo">

          <tr>
            <td>
            <a href="show?idPasoGuiaTramite=${bo.id}">
            ${bo.id}
            </a>
            </th>
            <c:forEach items="${mostrar}" varStatus="status" var="campo">
		  ...
		  <c:if test="${'nombre' eq campo}">
            ...
            <a onclick="return confirm('Esta seguro que desea eliminar?')" href="delete?idPasoGuiaTramite=${bo.id}"><img class="minibutton" alt="Eliminar" src="<c:url value='/images/icons/cross.png'/>"/></a>
            </sec:authorize>
            </td>
          </tr>
      </c:forEach>
...
<div id='botonera'>
  ...
  <sec:authorize ifAllGranted="ROLE_WRITE_PasoGuiaTramite">
      ...
      <a class="button" href="create"><img src="<c:url value='/images/icons/add.png'/>"/>Nuevo Paso Guia Tramite</a>
