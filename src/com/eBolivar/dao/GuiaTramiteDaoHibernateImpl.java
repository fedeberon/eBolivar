...
public class GuiaTramiteDaoHibernateImpl extends HibernateDaoSupport {
	
	public GuiaTramite getObject(Integer id) {
		return (GuiaTramite) getHibernateTemplate().get(GuiaTramite.class, id);
	}

	public List <GuiaTramite> getObjects() {
		return getHibernateTemplate().find("from GuiaTramite order by id");
	}

	public void removeObject(Integer id) {
		Object object = getHibernateTemplate().load(GuiaTramite.class, id);
        getHibernateTemplate().delete(object);
	}

	public void saveObject(GuiaTramite object) {
		getHibernateTemplate().saveOrUpdate(object);		
	}
	
	public List searchObjects(SearchObject search) {
		DetachedCriteria criteria = DetachedCriteria.forClass(GuiaTramite.class);
