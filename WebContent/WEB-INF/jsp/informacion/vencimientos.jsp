<jsp:include page="../header.jsp"/>

<html>
<header>

    <script type="text/javascript">

        $(document).ready(function () {
            $('#cronogramaVencimientos').addClass('active')

        });

    </script>

</header>


<body>


<div style="margin-left: 80px; margin-right: 80px; padding-top:10px; background-color: white; ">
    <div class="page-header">
        <hr>
        <span class="titulo-descripcion" style="padding-left: 10px;">Calendario Impositivo > 2017</span>
    </div>

    <table class="table table-bordered" style="font-size: 12px;">

        <thead>
        <tr>
            <th>Impuesto/Mes Venc</th>
            <th>Enero</th>
            <th>Febrero</th>
            <th>Marzo</th>
            <th>Abril</th>
            <th>Mayo</th>
            <th>Junio</th>
            <th>Julio</th>
            <th>Agosto</th>
            <th>Septiembre</th>
            <th>Octubre</th>
            <th>Noviembre</th>
            <th>Diciembre</th>
        </tr>
        </thead>

        <tbody>
        <tr>
            <th>Servicios Retributivos</th>
            <td>20-01</td>
            <td>15-02</td>
            <td>15-03</td>
            <td>14-04</td>
            <td>15-05</td>
            <td>15-06</td>
            <td>14-07</td>
            <td>15-08</td>
            <td>15-09</td>
            <td>16-10</td>
            <td>16-11</td>
            <td>15-12</td>
        </tr>

        <tr>
            <th>Servicios Sanitarios</th>
            <td>20-01</td>
            <td>15-02</td>
            <td>15-03</td>
            <td>14-04</td>
            <td>15-05</td>
            <td>15-06</td>
            <td>14-07</td>
            <td>15-08</td>
            <td>15-09</td>
            <td>16-10</td>
            <td>16-11</td>
            <td>15-12</td>
        </tr>

        <tr>
            <th>Red Vial</th>
            <td>20-01</td>
            <td>-</td>
            <td>15-03</td>
            <td>-</td>
            <td>15-05</td>
            <td>-</td>
            <td>14-07</td>
            <td>-</td>
            <td>15-09</td>
            <td>-</td>
            <td>16-11</td>
            <td>-</td>
        </tr>
        <tr>
            <th>Seguridad e Higiene</th>
            <td>-</td>
            <td>-</td>
            <td>20-03</td>
            <td>-</td>
            <td>20-05</td>
            <td>-</td>
            <td>20-07</td>
            <td>-</td>
            <td>20-09</td>
            <td>-</td>
            <td>20-11</td>
            <td>-</td>
        </tr>

        <tr>
            <th>Seguridad e Higiene - DDJJ</th>
            <td>-</td>
            <td>-</td>
            <td>20-03</td>
            <td>-</td>
            <td>22-05</td>
            <td>-</td>
            <td>20-07</td>
            <td>-</td>
            <td>20-09</td>
            <td>-</td>
            <td>20-11</td>
            <td>-</td>
        </tr>

        <tr>
            <th>Patente Automotor</th>
            <td>-</td>
            <td>-</td>
            <td>-</td>
            <td>-</td>
            <td>-</td>
            <td>15-06</td>
            <td>-</td>
            <td>-</td>
            <td>15-09</td>
            <td>-</td>
            <td>-</td>
            <td>15-12</td>
        </tr>
        <tr>
            <th>Patenete Motos</th>
            <td>-</td>
            <td>-</td>
            <td>-</td>
            <td>-</td>
            <td>-</td>
            <td>15-06</td>
            <td>-</td>
            <td>-</td>
            <td>-</td>
            <td>-</td>
            <td>-</td>
            <td>-</td>
        </tr>

        </tbody>
    </table>



    <table class="table table-bordered" style="font-size: 12px;">
        <thead>
        <tr><th colspan="13">Servicio de Guardia urbana y rural, monitoreo por camara de vigilancia y defensa civil</th></tr>
        <tr>
            <th>Impuesto/Mes Venc</th>
            <th>Enero</th>
            <th>Febrero</th>
            <th>Marzo</th>
            <th>Abril</th>
            <th>Mayo</th>
            <th>Junio</th>
            <th>Julio</th>
            <th>Agosto</th>
            <th>Septiembre</th>
            <th>Octubre</th>
            <th>Noviembre</th>
            <th>Diciembre</th>
        </tr>
        </thead>

        <tbody>
            <tr>
                <th>Inmuebles Urbanos</th>
                <td>20-01</td>
                <td>15-02</td>
                <td>15-03</td>
                <td>14-04</td>
                <td>15-05</td>
                <td>15-06</td>
                <td>14-07</td>
                <td>15-08</td>
                <td>15-09</td>
                <td>16-10</td>
                <td>16-11</td>
                <td>15-12</td>
            </tr>

            <tr>
                <th>Inmuebles Rurales</th>
                <td>25-01</td>
                <td>-</td>
                <td>15-03</td>
                <td>-</td>
                <td>16-05</td>
                <td>-</td>
                <td>14-07</td>
                <td>-</td>
                <td>15-09</td>
                <td>-</td>
                <td>16-11</td>
                <td>-</td>
            </tr>
        </tbody>
    </table>

    <table class="table table-bordered" style="font-size: 12px;">
        <thead>
        <tr><th colspan="13">Estaci&oacute;n de &Oacute;mnibus "Maria Teresa Josefa Rodriguez del Toro y Alaiza"</th></tr>
        <tr>
            <th>Impuesto/Mes Venc</th>
            <th>Enero</th>
            <th>Febrero</th>
            <th>Marzo</th>
            <th>Abril</th>
            <th>Mayo</th>
            <th>Junio</th>
            <th>Julio</th>
            <th>Agosto</th>
            <th>Septiembre</th>
            <th>Octubre</th>
            <th>Noviembre</th>
            <th>Diciembre</th>
        </tr>
        </thead>

        <tbody>
        <tr>
            <th>Derecho de Uso y Ocupaci&oacute;n</th>
            <td>-</td>
            <td>15-02</td>
            <td>-</td>
            <td>14-04</td>
            <td>-</td>
            <td>15-06</td>
            <td>-</td>
            <td>15-08</td>
            <td>-</td>
            <td>16-12</td>
            <td>-</td>
            <td>15-12</td>
        </tr>

        <tr>
            <th>Derecho de Mantenimiento</th>
            <td>-</td>
            <td>15-02</td>
            <td>-</td>
            <td>14-04</td>
            <td>-</td>
            <td>15-06</td>
            <td>-</td>
            <td>15-08</td>
            <td>-</td>
            <td>16-12</td>
            <td>-</td>
            <td>15-12</td>
        </tr>
        </tbody>
    </table>
    <hr>
</div>
</body>
</html>

<jsp:include page="../bottom.jsp"/>